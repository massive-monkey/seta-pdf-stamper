 ...
 * @version    $Id: Cmap.php 1753 2022-06-28 14:42:54Z maximilian.kresse $
 */
...
class SetaPDF_Core_Font_Cmap implements SetaPDF_Core_Font_Cmap_CmapInterface
{
     ...
     *
     * @param string $name
     * @param bool $cache This parameter will write a serialized version into the cmaps folder (only on PHP >= 7).
     *
    ...
    {
        $name = (string) preg_replace('/[^a-z\d_\-]/i', '', $name);
        ...
        // we only allow caching on PHP >= 7
        $phpBelow7 = version_compare(PHP_VERSION, '7.0.0') < 0;
        if (!$phpBelow7 && file_exists($path . '.cache') && is_readable($path . '.cache')) {
            $cmap = unserialize(file_get_contents($path . '.cache'), ['allowed_classes' => [self::class]]);
        } else {
            $cmap = self::create(new SetaPDF_Core_Reader_File($path));
            if (!$phpBelow7 && $cache && is_writable($dir)) {
        ...
        while (($token = $tokenizer->readToken()) !== false) {
            switch ($token) {
                ...
                case 'begincodespacerange':
                    $values = [];
                    while (($value = self::_readValue($tokenizer)) !== 'endcodespacerange' && $value !== false) {
                        $values[] = $value;
                    }
                ...
                case 'beginbfchar':
                    $values = [];
                    while (($value = self::_readValue($tokenizer)) !== 'endbfchar' && $value !== false) {
                        $values[] = $value;
                    }
                ...
                case 'beginbfrange':
                    $values = [];
                    while (($value = self::_readValue($tokenizer)) !== 'endbfrange' && $value !== false) {
                        $values[] = $value;
                    }
                ...
                case 'begincidchar':
                    $values = [];
                    while (($value = self::_readValue($tokenizer)) !== 'endcidchar' && $value !== false) {
                        $values[] = $value;
                    }
                ...
                case 'begincidrange':
                    $values = [];
                    while (($value = self::_readValue($tokenizer)) !== 'endcidrange' && $value !== false) {
                        $values[] = $value;
                    }
    ...
    protected function _lookup($type, $src)
    {
        ...
        foreach ($sizes AS $size) {
            /* walk backwards to get the latest definitions first
             * e.g.
    ...
    {
        $table = $this->_mappings[$type];
        $src = array_search($dest, $table['single'], true);
        if ($src !== false) {
            return $src;
        }

        $src = array_search($dest, $this->_lookUps[$type], true);
        if ($src !== false) {
            return $src;
        }
        ...
        if ($type === 'cid') {
            ...
            $destSuffixValue = ord($dest[strlen($dest) - 1]);
            ...
            foreach ($table['range'] AS $size => $entries) {
                foreach ($entries AS $entry) {
                    if (is_array($entry[2])) {
                        ...
                        $startValue = ord($_entry[strlen($_entry) - 1]);
    ...
    public function lookup($src)
    {
        if ($this->_cidMap !== null) {
            ...
            $src = (string)SetaPDF_Core_Encoding::unicodePointToUtf16Be($cid);
        }
