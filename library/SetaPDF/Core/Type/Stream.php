 ...
 * @version    $Id: Stream.php 1782 2022-09-28 12:55:13Z jan.slabon $
 */
...
class SetaPDF_Core_Type_Stream
    ...
    implements SplObserver, SetaPDF_Core_Canvas_StreamProxyInterface, SetaPDF_Core_Type_BypassSecHandlerInterface
{
    /**
    ...
    {
        return self::_ensureType(self::class, $stream, 'Stream value expected.');
    }
    ...
    public function getStream($filtered = false)
    {
        ...
        // remove filter
        if ($filtered === false && $this->_filtered === true) {
    ...
    public function setBypassSecHandler($bypassSecHandler = true)
    {
        ...
        if ($this->_bypassSecHandler === true && isset($this->_encrypted) && $this->hasCryptFilter() === false) {
    ...
    {
        if (isset($this->_encrypted) && $this->hasCryptFilter() === false) {
            ...
            if ($secHandler !== null) {
                ...
                $type = $this->getValue()->getValue('Type');
                if ($type && $type->getValue() === 'Metadata') {
                    if ($secHandler->getEncryptMetadata() === false) {
                        return $this->_stream;
                    }

                    // let's check for unencrypted metadata (there are faulty documents on the road)
                    if (strpos($this->_stream, '<?xpacket') === 0) {
                        return $this->_stream;
                    }
                }
    ...
    protected function _applyFilter($stream, $encode = false, SetaPDF_Core_Document $pdfDocument = null)
    {
        ...
        foreach ($filters AS $key => $filter) {
            ...
            switch ($filterName) {
                ...
                case 'Crypt':
                    // Filter is "Identity"
                    if ($decodeParam === null || !$decodeParam->offsetExists('Crypt')) {
                        break;
                    }
    ...
    protected function _preProcess(SetaPDF_Core_Document $pdfDocument)
    {
        ...
        $type = $this->getValue()->getValue('Type');
        $doNotEncrypt = ($secHandler && $type && $type->getValue() === 'Metadata' && $secHandler->getEncryptMetadata() === false);
        $shouldEncrypt = ($secHandler && $doNotEncrypt === false && $this->hasCryptFilter() === false && $this->_bypassSecHandler === false);
        ...
        if (!isset($this->_encrypted)) {
            ...
            if (!(
                $pdfDocument->getInstanceIdent() === $owningDocument->getInstanceIdent()
                    ...
                    $secHandlerIn !== null && $secHandler !== null &&
