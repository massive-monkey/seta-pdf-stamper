 ...
 * @version    $Id: AbstractHandler.php 1782 2022-09-28 12:55:13Z jan.slabon $
 */
...
abstract class SetaPDF_Core_SecHandler_AbstractHandler
{
    ...
    )
    {
        ...
        foreach ($encryptionDictionary AS $value) {
            if ($value instanceof SetaPDF_Core_Type_BypassSecHandlerInterface) {
                $value->setBypassSecHandler();
            }
        }

        // define the standard key length
        $keyLength = SetaPDF_Core_Type_Dictionary_Helper::getValue($encryptionDictionary, 'Length', 40, true);

        $this->_keyLength = $keyLength / 8;

        // Crypt Filters / V == 4
        $cryptFilters = SetaPDF_Core_Type_Dictionary_Helper::getValue($encryptionDictionary, 'CF');
        if ($cryptFilters instanceof SetaPDF_Core_Type_Dictionary) {
            $streamFilterName = SetaPDF_Core_Type_Name::ensureType(
                SetaPDF_Core_Type_Dictionary_Helper::getValue($encryptionDictionary, 'StmF')
            )->getValue();

            // TODO: This can be "Identity" which is a predefined crypt filter
            $streamFilter = SetaPDF_Core_Type_Dictionary::ensureType(
                SetaPDF_Core_Type_Dictionary_Helper::getValue($cryptFilters, $streamFilterName)
            );

            $cryptFilterMethod = SetaPDF_Core_Type_Dictionary_Helper::getValue($streamFilter, 'CFM', null, true);
            $keyLength = SetaPDF_Core_Type_Dictionary_Helper::getValue($streamFilter, 'Length', $this->_keyLength, true);
            ...
            /**
             * @var SetaPDF_Core_Type_Dictionary $stringFilter
             */
            $stringFilterName = SetaPDF_Core_Type_Name::ensureType(
                SetaPDF_Core_Type_Dictionary_Helper::getValue($encryptionDictionary, 'StrF')
            )->getValue();

            $stringFilter = SetaPDF_Core_Type_Dictionary::ensureType(
                SetaPDF_Core_Type_Dictionary_Helper::getValue($cryptFilters, $stringFilterName)
            );

            $cryptFilterMethod = SetaPDF_Core_Type_Dictionary_Helper::getValue($stringFilter, 'CFM', null, true);
            $keyLength = SetaPDF_Core_Type_Dictionary_Helper::getValue($stringFilter, 'Length', $this->_keyLength, true);
        ...
        $this->_encryptMetadata = SetaPDF_Core_Type_Dictionary_Helper::getValue(
    ...
    {
        $algoCode = SetaPDF_Core_Type_Dictionary_Helper::getValue($this->_encryptionDictionary, 'V', '?', true);
